...
class Net(nn.Module):
    ...
    def generate_from_style(self, style_tensor):
        """
        Gunakan style tensor hasil edit (shape: [1, 18, 512]) untuk menghasilkan gambar akhir.
        """
        with torch.no_grad():
            image, _ = self.generator([style_tensor], input_is_latent=True, randomize_noise=False)
        return image
