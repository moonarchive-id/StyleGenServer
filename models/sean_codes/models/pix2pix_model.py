    ...
    def preprocess_input(self, data):
        ...
        if self.use_gpu():
            for param in ['label', 'instance', 'image']:
                if param in data:
                    data[param] = data[param].to(torch.device('cpu'))

            if 'obj_dic' in data:
                for idx in range(19):
                    if data['obj_dic'][str(idx)]['ACE'].device.type == 'cpu':
                        data['obj_dic'][str(idx)]['ACE'] = data['obj_dic'][str(idx)]['ACE'].to(torch.device('cpu'))
        # create one-hot label map
        label_map = data['label']
        bs, _, h, w = label_map.size()
        nc = self.opt.label_nc + 1 if self.opt.contain_dontcare_label \
            else self.opt.label_nc
        input_label = torch.zeros((bs, nc, h, w), dtype=torch.float32)
...
def load_average_feature():
    ...
    for i in range(19):
        ...
        for style_code_path in average_category_list:
            style_code_array = np.load(os.path.join(average_style_code_folder, str(i), style_code_path + '.npy'))
            input_style_dic[str(i)][style_code_path] = torch.from_numpy(style_code_array).to(torch.device('cpu'))
