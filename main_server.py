...
app = FastAPI(
    ...
    description="HairFast Studio Pro adalah aplikasi AI untuk mengedit wajah dan swap rambut dengan teknologi HairFast.",
    version="HairFast-Studio-V17.0-Zeroconf",
)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        print(f"Pendaftaran layanan Zeroconf 'HairFast API' berhasil!", flush=True)
...
def local_warp_effect(image, center, radius, strength):
    ...
    for y in range(h):
        for x in range(w):
            dist_x, dist_y = x - center[0], y - center[1]
            ...
            if distance < radius:
                ...
                map_x[y, x], map_y[y, x] = float(x), float(y)
...
def edit_wajah_mediapipe(face_pil, p_skin, p_eyes, p_nose_bridge, p_nose_tip, p_lips, p_forehead_width, p_forehead_height, p_jaw):
    try:
        ...
        original_cv = cv2.cvtColor(np.array(face_pil), cv2.COLOR_RGB2BGR)
        edited_cv = original_cv.copy()
        ...
        with mp_face_mesh.FaceMesh(static_image_mode=True, max_num_faces=1, refine_landmarks=True, min_detection_confidence=0.5) as face_mesh:
            ...
            if p_skin != 0:
                ...
                if p_skin > 0:
                    low_pass = cv2.GaussianBlur(edited_cv, (21, 21), 0)
                    high_pass = cv2.subtract(edited_cv, low_pass)
                    smoothed_low_pass = cv2.bilateralFilter(low_pass, int(p_skin*1.5)*2+1, 75, 75)
                    modified_face = cv2.add(smoothed_low_pass, high_pass)
                    blend_ratio = p_skin / 25.0
                else:
                    amount = abs(p_skin) / 15.0
                    blurred = cv2.GaussianBlur(edited_cv, (0,0), 3)
                    modified_face = cv2.addWeighted(edited_cv, 1.0 + amount, blurred, -amount, 0)
                    blend_ratio = amount * 0.75
            ...
            if p_eyes != 0:
                ...
                radius = int(w * 0.08)
                left_eye_center = (int(landmarks[473].x * w), int(landmarks[473].y * h))
                edited_cv = local_warp_effect(edited_cv, left_eye_center, radius, strength)
                right_eye_center = (int(landmarks[468].x * w), int(landmarks[468].y * h))
                edited_cv = local_warp_effect(edited_cv, right_eye_center, radius, strength)
                
            if p_nose_bridge != 0:
                strength = p_nose_bridge * 0.035
                radius = int(h * 0.22)
                bridge_center = (int(landmarks[168].x * w), int(landmarks[168].y * h))
                edited_cv = local_warp_effect(edited_cv, bridge_center, radius, strength)
            
            if p_nose_tip != 0:
                strength = p_nose_tip * 0.05
                radius = int(h * 0.12)
                tip_center = (int(landmarks[4].x * w), int(landmarks[4].y * h))
                edited_cv = local_warp_effect(edited_cv, tip_center, radius, strength)
            ...
            if p_jaw != 0:
                strength = p_jaw * 0.055
                radius = int(h * 0.25)
                chin_center = (int(landmarks[152].x * w), int(landmarks[152].y * h))
                edited_cv = local_warp_effect(edited_cv, chin_center, radius, strength)

            return Image.fromarray(cv2.cvtColor(edited_cv, cv2.COLOR_BGR2RGB)), "✅ Wajah berhasil diedit."
    except Exception as e:
        traceback.print_exc()
        return None, f"❌ Terjadi kesalahan saat edit wajah: {e}"
...
async def preview_face_api(face_image: UploadFile = File(...), p_skin: float = Form(0.0), p_eyes: float = Form(0.0), p_nose_bridge: float = Form(0.0), p_nose_tip: float = Form(0.0), p_lips: float = Form(0.0), p_forehead_width: float = Form(0.0), p_forehead_height: float = Form(0.0), p_jaw: float = Form(0.0)):
    face_pil = Image.open(io.BytesIO(await face_image.read()))
    result_pil, status_msg = edit_wajah_mediapipe(face_pil, p_skin, p_eyes, p_nose_bridge, p_nose_tip, p_lips, p_forehead_width, p_forehead_height, p_jaw)
    if result_pil is None: raise HTTPException(status_code=500, detail=status_msg)
    byte_io = io.BytesIO()
    result_pil.save(byte_io, 'JPEG')
    byte_io.seek(0)
    return StreamingResponse(byte_io, media_type="image/jpeg")

def unified_swap_logic(face_pil, shape_pil, color_pil, edit_kwargs):
    try:
        device = hair_fast_model.args.device
        face_tensor = F.resize(F.to_tensor(face_pil), [1024, 1024]).to(device)
        shape_tensor = F.resize(F.to_tensor(shape_pil), [1024, 1024]).to(device)
        color_tensor = F.resize(F.to_tensor(color_pil), [1024, 1024]).to(device)
...
with gradio_ui:
    gr.Markdown("# HairFast Studio Pro")
    with gr.Tabs() as tabs:
        with gr.Tab(label="🖌️ Editor Wajah", id=0):
            with gr.Row():
                ...
                with gr.Accordion("Kontrol Editor", open=True):
                    with gr.Row():
                        with gr.Column():
                            ...
                            nose_bridge_slider = gr.Slider(-15, 15, step=1, label="Batang Hidung", value=0, info="Kiri: ramping, Kanan: lebar.")
                            nose_tip_slider = gr.Slider(-15, 15, step=1, label="Ujung Hidung", value=0, info="Kiri: ramping, Kanan: lebar.")
                        with gr.Column():
                            ...
                            jaw_slider = gr.Slider(-15, 15, step=1, label="Bentuk Dagu", value=0, info="Kiri: tajam (V), Kanan: Lebar.")
        ...
        with gr.Tab(label="✂️ Swap Rambut", id=1):
            with gr.Row():
            ...
            swap_output_image = gr.Image(label="Hasil Akhir", type="pil", interactive=False, height=512)
    ...
    def swap_hair_gradio_wrapper(face_img, shape_img, color_img):
        if not all([face_img, shape_img, color_img]): return None, "❌ Semua gambar wajib diunggah."
        edit_kwargs = {'gender': 0.0, 'age': 0.0}
        return unified_swap_logic(face_img, shape_img, color_img, edit_kwargs)
    
    swap_run_button.click(fn=swap_hair_gradio_wrapper, inputs=[swap_face_input, swap_shape_input, swap_color_input], outputs=[swap_output_image, swap_status_text])

aapp = gr.mount_gradio_app(app, gradio_ui, path="/web")

def run_gradio_share_server():
    print("\n[Gradio Share] Mencoba membuat link publik yang aman...")
    gradio_ui.launch(share=True, server_port=7861)

if __name__ == '__main__':
    gradio_thread = threading.Thread(target=run_gradio_share_server, daemon=True)
    gradio_thread.start()

    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        s.connect(('8.8.8.8', 1))
        true_ip_for_print = s.getsockname()[0]
    except Exception:
        true_ip_for_print = "127.0.0.1"

    print("\n" + "="*60)
    print("🚀 Server Studio Pro Siap Dijalankan! 🚀")
    print("✨ Menggabungkan Uvicorn (untuk Lokal/API) & Gradio Share (untuk Publik).")
    print(f"✅ [LOKAL & API] Server utama berjalan, Flutter bisa konek ke: http://{true_ip_for_print}:5000")
    print("="*60 + "\n")

    uvicorn.run(app, host='0.0.0.0', port=5000, log_level="info", reload=False, workers=1)